<?php
/**
 *  Petróleo na Amazônia não - Check if user is underage at Gravity Forms birthdate input
 */
$field = GFFormsModel::get_field($form, 1);
$date_type = $field->dateType;

add_filter('gform_field_validation_31_08', function ($result, $value, $form, $field) {
    if ($result['is_valid']) {
        if (is_array($value)) {
